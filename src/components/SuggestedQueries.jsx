...
export default function SuggestedQueries({ onSelect }) {
  return (
    <div className="px-6 pb-4">
      <p className="text-xs text-slate-400 font-mono uppercase tracking-wider mb-2">Try asking</p>
      <div className="flex flex-wrap gap-2">
        {SUGGESTIONS.map(s => (
          <button
            key={s.query}
            onClick={() => onSelect(s.query)}
            className="text-xs px-3 py-1.5 rounded-full bg-white border border-slate-200 text-slate-500 hover:text-slate-800 hover:border-slate-300 hover:shadow-sm transition-all duration-150"
          >
            {s.label}
          </button>
        ))}
